/**
 * mc.js - CreateJS Movie Clip Definitions
 * 
 * This file was generated by CreateJS and contains all
 * the graphic assets used in the Dice Wars game, including:
 * - Game UI elements (buttons, backgrounds, text)
 * - Dice representations for territories and battles
 * - Visual effects and animations
 * - Game screens (title, game over, victory)
 * 
 * The file defines MovieClip objects that can be instantiated in main.js
 * using lib.mc() and manipulated through the CreateJS API.
 */

(function (lib, img, cjs, ss) {

var p; // shortcut to reference prototypes (internal CreateJS optimization)
lib.webFontTxtFilters = {}; 

// Canvas and animation properties for the entire library:
lib.properties = {
	width: 840,       // Base canvas width (scaled by the responsive system in main.js)
	height: 840,      // Base canvas height
	fps: 24,          // Animation framerate
	color: "#FFFFFF", // Background color
	opacity: 1.00,    // Overall opacity
	webfonts: {},     // Web font tracking
	manifest: []      // Asset manifest (empty because assets are embedded)
};



// Sprite sheet metadata (empty in this implementation)
lib.ssMetadata = [];

/**
 * Web Font handling function
 * Called when a web font becomes available for use
 * Updates text elements that use this font family
 * 
 * @param {string} family - The font family name that has been loaded
 */
lib.webfontAvailable = function(family) { 
	// Mark this font as available
	lib.properties.webfonts[family] = true;
	
	// Get any text objects using this font that need cache updating
	var txtFilters = lib.webFontTxtFilters && lib.webFontTxtFilters[family] || [];
	
	// Update all text objects using this font
	for(var f = 0; f < txtFilters.length; ++f) {
		txtFilters[f].updateCache();
	}
};

// Symbol definitions (graphic assets used in the game):

/**
 * YOU WIN Text Display
 * 
 * This MovieClip creates the "YOU WIN" text shown after a player victory.
 * It includes the text itself along with shadow and highlight effects.
 * Used in main.js during the win sequence animation.
 */
 ...
 * Main Game Logo
 * 
 * This MovieClip creates the "DICE WARS" game logo.
 * It consists of multiple layers that give the logo depth and visual appeal:
 * - The main text in orange/gold
 * - Highlight effects for shine
 * - Shadow layers for depth
 * 
 * Used in the title screen and potentially other parts of the UI.
 */
...
/**
 * 3-Value Dice Graphic (Orange/Amber Color Theme)
 * 
 * This MovieClip creates a 3D dice with 3 dots visible on its top face.
 * Features:
 * - 3D perspective with multiple face details
 * - Lighting effects and shadows
 * - Dot patterns representing the value "3"
 * - Orange/amber color scheme for player identification
 * 
 * Used to represent dice with value 3 on territories controlled by a specific player.
 */
...
/**
 * 2-Value Dice Graphic (Pink/Magenta Color Theme)
 * 
 * This MovieClip creates a 3D dice with 2 dots visible on its top face.
 * Features:
 * - 3D perspective with multiple face details
 * - Lighting effects and shadows
 * - Dot patterns representing the value "2"
 * - Pink/magenta color scheme for player identification
 * 
 * Used to represent dice with value 2 on territories controlled by a specific player.
 */
...
/**
 * CreateJS library initialization
 * 
 * This final code initializes the CreateJS library and exports necessary variables
 * to the global scope for use in main.js. The library contains all graphical assets
 * needed for the Dice Wars game, organized as MovieClip objects that can be 
 * instantiated and controlled via CreateJS.
 * 
 * These assets include:
 * - Player-specific dice graphics with different colors and values
 * - UI elements like buttons, text displays, and backgrounds
 * - Game screens (title, victory, game over)
 * - Effects and animations
 */
})(lib = lib||{}, images = images||{}, createjs = createjs||{}, ss = ss||{});

// Export variables to the global scope for use in main.js
