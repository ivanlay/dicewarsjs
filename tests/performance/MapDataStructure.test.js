...
describe('AreaData Performance Comparison', () => {
  ...
  test('Comparison: Setting adjacency', () => {
    ...
    console.log(`Setting adjacency - Array: ${arrayTime.toFixed(2)}ms, Map: ${mapTime.toFixed(2)}ms`);
  ...
  test('Comparison: Checking adjacency', () => {
    ...
    const { area: arrayAreaData, adjacentIds } = createTerritoryWithRandomAdjacencies(ArrayAreaData, 50, maxAreaId);
    ...
    console.log(`Checking adjacency - Array: ${arrayTime.toFixed(2)}ms, Map: ${mapTime.toFixed(2)}ms`);
  });
  ...
  test('Comparison: Finding all adjacent areas', () => {
    ...
    const { area: arrayAreaData, adjacentIds } = createTerritoryWithRandomAdjacencies(ArrayAreaData, 200, maxAreaId);
    ...
    console.log(`Finding adjacent areas - Array: ${arrayTime.toFixed(2)}ms, Map: ${mapTime.toFixed(2)}ms`);
  });
  ...
  test('Comparison: Memory usage with large number of areas', () => {
    ...
    for (let i = 0; i < areaCount; i++) {
      for (let j = 0; j < areaCount; j++) {
        if (i !== j && Math.random() < adjacencyPercentage) {
          if (j < 32) { // Array has fixed size of 32
    ...
    console.log(`Average adjacencies - Array: ${arrayAdjacenciesAvg.toFixed(2)}, Map: ${mapAdjacenciesAvg.toFixed(2)}`);
    ...
    console.log(`Large-scale access - Array: ${arrayAccessTime.toFixed(2)}ms, Map: ${mapAccessTime.toFixed(2)}ms`);
