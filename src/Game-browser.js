/**
 * Browser-compatible version of Game.js with relative imports
 * This file is generated from the original Game.js with all aliases resolved
 */
...
export class Game {
  ...
  ai = [
    ...
    null, // AI functions will be assigned during configuration
    null,
    null,
    null,
    null,
    ...
    if (typeof window !== 'undefined' && 'requestIdleCallback' in window) {
      window.requestIdleCallback(
        () => {
          ...
          import('./mechanics/battleResolution.js').catch(err =>
            console.warn('Error preloading battle mechanics:', err)
          );

          // Preload map generation for when starting a new game
          setTimeout(() => {
            import('./mechanics/mapGenerator.js').catch(err =>
              console.warn('Error preloading map generator:', err)
            );
          }, 1000);

          /*
           * Load debugging tools in development mode
           * Check if we're in development mode (localhost or development domain)
           */
          const isDevelopment =
